...
class Title {
  ...
  createMesh() {
    ...
    const textScale = screenWidth < 768 ? 0.08 : 0.15 // Smaller text on mobile
    ...
    const offsetY = screenWidth < 768 ? 0.03 : 0.05
    this.mesh.position.y =
      -this.plane.scale.y * 0.5 - textHeightScaled * 0.5 - offsetY
...
class App {
  ...
  private setInitialScrollOffset() {
    if (!this.medias.length) return
    const w = this.medias[0].width || 0
    this.scroll.current = this.scroll.target = w * (this.medias.length / 2)
  }
  createMedias(
  ...
  onTouchUp() {
    ...
    // Auto resume after drag interaction with longer delay
    setTimeout(() => {
  ...
  onWheel(e: Event) {
    ...
    // Mark as user interacting but only briefly for wheel events
    this.userInteracting = true
    this.lastUserInteraction = Date.now()

    // Resume auto play after a shorter delay for wheel events
    setTimeout(() => {
      if (Date.now() - this.lastUserInteraction >= 800) {
        this.userInteracting = false
      }
    }, 1000)
  ...
  shouldAutoPlay(): boolean {
    ...
    // For drag interactions, wait longer before resuming
    if (this.userInteracting) return false

    // Resume auto play after shorter delay (800ms instead of 2000ms)
    const timeSinceLastInteraction = Date.now() - this.lastUserInteraction
    if (timeSinceLastInteraction < 800) return false
