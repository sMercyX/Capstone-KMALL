...
export default function StoreEditProductModal({
  ...
  const handleSelectMain = (index: number) => {
    ...
    // const ok = window.confirm("ต้องการตั้งรูปนี้เป็นรูปหลัก?")
    // if (!ok) return
    setMainIndex(index)
  }
  ...
  const handleDeleteImage = (index: number) => {
    ...
    const ok = window.confirm("ต้องการลบรูปภาพนี้ใช่หรือไม่?")
    if (!ok) return
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Note: image_url in product table is usually auto-updated by backend when primary image changes,
      // or we might need to send it. For now, let's send current main image URL if it's EXISTING.
      // But actually backend logic `normalizeProductPrimary` handles `is_primary`.
      // The `product` table `image_url` might be a cache or just the primary image.
      // Let's just update the fields we edit.
      ...
      // 4. Set Primary Image
      // We need to find which image is currently selected as mainIndex
      // mainIndex is an index in the `images` state array.
      ...
      if (mainImageItem) {
        ...
        if (mainImageItem.type === "EXISTING") {
          ...
          // We need to map the "NEW" images in `images` state to `uploadedNewImages`.
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
      <div className="w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-3xl bg-white shadow-2xl border border-gray-100 animate-in fade-in slide-in-from-bottom-4 duration-200">
        ...
        <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-10">
          {/* ===== LEFT: IMAGES ===== */}
          <div>
            <div className="w-full aspect-square rounded-2xl bg-[#f8f8f8] border border-gray-200 flex items-center justify-center overflow-hidden relative">
              {mainImageSrc ? (
                <img
                  src={mainImageSrc}
                  alt="product main"
            ...
            <div className="space-y-5 flex-1">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 disabled:bg-gray-100"
                >
